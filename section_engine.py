...
class SectionControl:
    ...
    def process(self, lat, lon, heading_deg, speed):
        """Головна логіка секційного контролю з розумним Look Ahead в поворотах"""
        ...
        # Проекція координат WGS84 у метри (UTM)
        ...
        # Фільтр шуму GPS: якщо зсув менше 5 см, ігноруємо крок
        ...
        # -----------------------------------------------------------------------------------
        # 1. РОЗУМНИЙ LOOK AHEAD (ВРАХОВУЄМО ПОВОРОТ ТА РЕЖИМИ ON/OFF)
        # -----------------------------------------------------------------------------------
        # Визначаємо, який час брати: якщо хоч одна секція зараз увімкнена — ми в процесі поливу,
        # отже наступна дія — можливе ВИМКНЕННЯ (OFF) на перекритті. Якщо все вимкнено — шукаємо ВВІМКНЕННЯ (ON).
        is_any_section_active = any(self.last_p1_list) # або дивимось на минулий стан
        
        if is_any_section_active:
            time_ahead = self.cfg.get("LOOK_AHEAD_OFF_TIME", 0.4) # Клапани вимикаються швидко
        else:
            time_ahead = self.cfg.get("LOOK_AHEAD_ON_TIME", 0.8)  # Насосу потрібен час накачати тиск

        speed_ms = speed / 3.6
        
        # Отримуємо turn_rate (градуси/сек), який вираховується в методе curve_compensation
        # (Для цього переконайтеся, що self.last_turn_rate зберігається в curve_compensation, див. нижче)
        turn_rate = getattr(self, 'last_turn_rate', 0.0) 
        
        # Якщо трактор реально крутить кермом (кут змінюється швидше ніж на 0.5 градуса в сек)
        if abs(turn_rate) > 0.5 and speed_ms > 0.2:
            omega = math.radians(turn_rate)
            # Курс, який буде у трактора через time_ahead секунд
            predicted_heading = heading + (omega * time_ahead)
            
            # Математика руху по дузі (Трактор зміщується по радіусу повороту)
            # Використовуємо класичний поворот навігаційної системи (0 = Північ)
            ...
            # Якщо їдемо прямо — стандартний лінійний винос вперед
        ...
        # -----------------------------------------------------------------------------------
        # 2. ПЕРЕВІРКА ПЕРЕКРИТТІВ ДЛЯ КОЖНОЇ СЕКЦІЇ
        # -----------------------------------------------------------------------------------
        ...
        for i, w in enumerate(widths):
            # Розрахунок нових точок штанги випередження (використовуємо look_ahead_heading!)
                    ...
                    if not self.covered_area.is_empty and not test_poly.is_empty:
                        if self.covered_area.intersects(test_poly):
                            ...
                            if (inter_area / test_poly.area) > min_overlap:
                                is_on = False  # Перекриття знайдено -> Вимикаємо
        ...
        # Зберігаємо історію для фронтенду
        ...
        if polys_to_save:
            try:
                ...
                print(f"Помилка обробки карти: {e}")
    ...
    def process_1(self, lat, lon, heading_deg, speed):
        ...
        for i, w in enumerate(widths):
            # Розрахунок нових передніх точок полігону секції
            p1 = self.get_section_point(pred_x, pred_y, heading, l_offset)
            p2 = self.get_section_point(pred_x, pred_y, heading, l_offset + w)
            curr_p1.append(p1)
            curr_p2.append(p2)

            # Формуємо задні точки (базуючись на минулому кроці або поточній позиції)
            if self.last_p1_list and len(self.last_p1_list) > i:
                p4, p3 = self.last_p1_list[i], self.last_p2_list[i]
            else:
                p4 = self.get_section_point(ux, uy, heading, l_offset)
                ...
                if mode == "ON": 
                    ...
                    if not self.covered_area.is_empty and not test_poly.is_empty:
                        if self.covered_area.intersects(test_poly):
                            inter_area = self.covered_area.intersection(test_poly).area
                            if (inter_area / test_poly.area) > min_overlap:
                                is_on = False  # Секція вимикається (знайшли перекриття)
        ...
        # Зберігаємо історію для відмальовки сліду на клієнті
        self.path_history.append([lat, lon, heading_deg, list(res_states)])
        if len(self.path_history) > 10000: 
            self.path_history.pop(0)

        # Оновлення геометрії карти поля
        if polys_to_save:
            try:
                self.covered_area = self.covered_area.union(unary_union(polys_to_save))
                
                # Оптимізація карти: робимо simplify і пишемо на диск рідше (раз на 150 кроків)
                if len(self.path_history) % 150 == 0:
                    self.covered_area = self.covered_area.simplify(0.05, preserve_topology=True)
                    self.save_to_disk()
            except Exception as e: 
